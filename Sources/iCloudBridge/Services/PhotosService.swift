...
class PhotosService: ObservableObject {
    ...
    // Check if album title looks like an auto-generated/imported album
    private func isAutoGeneratedAlbum(_ title: String?) -> Bool {
        guard let title = title else { return true }

        let trimmedTitle = title.trimmingCharacters(in: .whitespacesAndNewlines)

        // Quick string-based checks for common patterns
        let containsPatterns = ["Photo Stream", "Photo Booth", "Last Import"]
        for pattern in containsPatterns {
            if trimmedTitle.localizedCaseInsensitiveContains(pattern) {
                return true
            }
        }

        // Check for "Roll N" pattern
        if trimmedTitle.hasPrefix("Roll ") {
            return true
        }

        // Check for "Untitled" albums
        if trimmedTitle.hasPrefix("Untitled") {
            return true
        }

        // Regex patterns for date-based albums from iPhoto imports
        let datePatterns = [
            // "Nov 12, 2011" or "November 12, 2011" with optional " (1)" suffix
            "^(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[a-z]* \\d{1,2}, \\d{4}( \\(\\d+\\))?$",
            // "December 2015" or "Dec 2015" with optional suffix
            "^(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[a-z]* \\d{4}( \\(\\d+\\))?$",
        ...
        for pattern in datePatterns {
            if let regex = try? NSRegularExpression(pattern: pattern, options: .caseInsensitive) {
                ...
                if regex.firstMatch(in: trimmedTitle, options: [], range: range) != nil {
                    return true
                }
            }
        }
    ...
    func loadAlbums() {
        ...
        userAlbums.enumerateObjects { collection, _, _ in
            if !self.isAutoGeneratedAlbum(collection.localizedTitle) {
